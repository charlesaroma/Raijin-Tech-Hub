...
const GalleryGrid = ({ activeCategory, setActiveCategory, currentCategory, currentImages, imagesRef, openLightbox }) => {
  return (
    <>
      ...
      <p className="text-center text-(--color-primary-600) text-xs mb-8">
        ...
        {currentImages && currentImages.length > 0 ? (
          currentImages.map((image, index) => (
            <motion.div
              ...
              className="group relative aspect-square rounded-2xl overflow-hidden cursor-pointer border-2 border-(--color-primary-500)/20 hover:border-(--color-primary-500) transition-all duration-300"
            ...
            <div className="absolute inset-0 bg-linear-to-t from-black/80 via-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300">
              <div className="absolute bottom-0 left-0 right-0 p-4">
                ...
                <span className="inline-block px-3 py-1 bg-(--color-primary-500) text-white rounded-full text-xs">
                  {image.category}
                </span>
              </div>
              ...
              <div className="absolute top-4 right-4">
                <div className="w-10 h-10 bg-white/90 rounded-full flex items-center justify-center text-(--color-primary-600) transform scale-0 group-hover:scale-100 transition-transform duration-300">
          ...
          <div className="text-center py-20 col-span-full">
            <Icon icon="mdi:image-off" className="text-6xl text-(--color-primary-500)/30 mx-auto mb-4" />
      ...
      {currentImages.length === 0 && currentCategory && (
        <div className="text-center py-20">
          <Icon icon="mdi:image" className="text-6xl text-(--color-primary-500)/30 mx-auto mb-4" />
          <p className="text-xl text-(--color-primary-500)/60">
