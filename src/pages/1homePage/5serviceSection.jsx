...
const ServicesSection = () => {
  return (
    <section className="py-8 sm:py-12 lg:py-16 bg-(--color-bg-secondary) relative overflow-hidden">
      <div className="max-w-5xl mx-auto px-4 sm:px-6">
        {/* Header */}
        <div className="text-center mb-8 sm:mb-10 lg:mb-12">
          <div className="inline-block">
            <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-(--color-primary-500) mb-3 relative px-2">
              Our Services
              <div className="absolute -bottom-1 left-0 w-full h-0.5 bg-linear-to-r from-transparent via-(--color-primary-500) to-transparent"></div>
            </h2>
          </div>
          <p className="mt-4 text-sm sm:text-sm text-(--color-off-black-500) max-w-2xl mx-auto px-4">
            Discover the range of services we offer to help your business grow
          ...
          {services.map((service, index) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className="absolute inset-0 opacity-[0.02] group-hover:opacity-[0.05] transition-opacity duration-500 rounded-2xl sm:rounded-3xl"
                  style={{
                ...
                {/* Corner decorations */}
                <div className="absolute top-0 left-0 w-8 h-8 sm:w-12 sm:h-12 border-t-2 border-l-2 border-(--color-primary-500)/0 group-hover:border-(--color-primary-500) rounded-tl-2xl sm:rounded-tl-3xl transition-all duration-300"></div>
                <div className="absolute bottom-0 right-0 w-8 h-8 sm:w-12 sm:h-12 border-b-2 border-r-2 border-(--color-primary-500)/0 group-hover:border-(--color-primary-500) rounded-br-2xl sm:rounded-br-3xl transition-all duration-300"></div>

                {/* Content */}
                <div className="relative h-full flex flex-col justify-center z-10">
                  {/* Icon with animated ring */}
                  <div
                    className={`relative mb-3 sm:mb-4 ${isLarge ? "lg:mb-6" : ""} w-fit`}
                  >
                    <div className="absolute inset-0 rounded-2xl bg-(--color-primary-500)/10 blur-xl scale-0 group-hover:scale-150 transition-transform duration-500"></div>
                    <div
                      className={`relative w-12 h-12 sm:w-14 sm:h-14 ${isLarge ? "lg:w-16 lg:h-16" : ""} flex items-center justify-center text-(--color-primary-500) transform transition-all duration-500 group-hover:scale-110 group-hover:-rotate-6`}
                    >
                      {service.icon}
                    </div>
                    {/* Animated ring */}
                    <div className="absolute inset-0 rounded-full border-2 border-(--color-primary-500)/0 group-hover:border-(--color-primary-500)/50 scale-100 group-hover:scale-125 transition-all duration-500"></div>
                  </div>
                  ...
                  <h3
                    ...
                    mb-1.5
                    font-bold text-(--color-primary-500) 
                  ...
                  <p
                    ...
                    text-(--color-primary-500)/80 dark:text-(--color-primary-400)/80 
                    leading-relaxed
                    transition-all duration-300 group-hover:text-(--color-primary-500)
                  `}
                  >
                    {service.description}
                  </p>
                ...
                <div className="absolute inset-0 overflow-hidden rounded-2xl sm:rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none">
                  <div className="absolute w-full h-[2px] bg-linear-to-r from-transparent via-(--color-primary-500) to-transparent -translate-y-full group-hover:translate-y-[200px] sm:group-hover:translate-y-[220px] transition-transform duration-1500 ease-linear"></div>
                </div>
              </div>
            );
          })}
